...
# hyperparameters
batch_size = 64 # кол-во независимых последовательностей, обрабатываемых одновременно
block_size = 256 # максимальная длина контекста для предсказания токена
max_iters = 5000
eval_interval = 500
learning_rate = 3e-4
...
# все уникальные символы, предситавленные в тексте
...
class Head(nn.Module):
    ...
    def forward(self, x):
        # ввод размерности (batch, time-step, channels)
        # вывод размерности (batch, time-step, head size)
        B,T,C = x.shape
        k = self.key(x)   # (B,T,hs)
        q = self.query(x) # (B,T,hs)
        # вычисление "очков родства" ("affinities")
        wei = q @ k.transpose(-2,-1) * k.shape[-1]**-0.5 # (B, T, hs) @ (B, hs, T) -> (B, T, T)
        wei = wei.masked_fill(self.tril[:T, :T] == 0, float('-inf')) # (B, T, T)
        wei = F.softmax(wei, dim=-1) # (B, T, T)
        wei = self.dropout(wei)
        # выполнить взвешенное объединение значений
...
class Block(nn.Module):
    ...
    def __init__(self, n_embd, n_head):
        # n_embd: измерение встраивания, n_head: количество голов, которое мы хотели бы получить
        super().__init__()
...
class GPTLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.ln_f = nn.LayerNorm(n_embd) # конечный слой нормализации
    ...
    def generate(self, idx, max_new_tokens):
        ...
        for _ in range(max_new_tokens):
            # обрезать idx до последнего токена размера блока (block_size)
...
m = model.to(device)
# вывод кол-ва параметров в модели
print(sum(p.numel() for p in m.parameters())/1e6, 'M parameters')

# создать оптимизатор PyTorch
